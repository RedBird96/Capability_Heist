...
module overmind::capability_heist {
    ...
    */
    public entry fun init(robber: &signer) {
        ...
        let (_, signer_capability) = 
                    account::create_resource_account(robber_address, SEED);

        // TODO: Create all the four capabilities
        let enterbank = EnterBank{};
        let takehostage = TakeHostage{};
        let getkeycard = GetKeycard{};
        let openvalue = OpenVault{};

        // TODO: Move Robber to the signer
        move_to(robber_address, Robber {
    ...
    */
    public entry fun enter_bank(robber: &signer, answer: String) acquires Robber {
        ...
        assert_answer_is_correct(ENTER_BANK_ANSWER, &answer);
    ...
    */
    public fun delegate_capability<Feature>(
        ...
        let robber_address = signer::address_of(robber);
        let bingo = borrow_global_mut<Robber>(robber_address);
        capabilities::acquire(robber, feature);
    }
    ...
    */
    ...
    inline fun assert_answer_is_correct(expected_answer: &vector<u8>, actual_answer: &String) {
        ...
        let value = aptos_hash::sha3_512(*string::bytes(actual_answer));
        assert!(expected_answer == value, ERROR_INCORRECT_ANSWER);
